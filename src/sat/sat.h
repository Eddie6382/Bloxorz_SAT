...
class SatSolver
{
   public : 
      ...
      void addOr3CNF(Var va, bool fa, Var vb, bool fb, Var vc, bool fc) {
         vec<Lit> lits;
      ...
      // fa/fb = true if it is inverted
      void addOr2CNF(Var va, bool fa, Var vb, bool fb) {
         vec<Lit> lits;
         Lit la = fa? ~Lit(va): Lit(va);
         Lit lb = fb? ~Lit(vb): Lit(vb);
      ...
      // fa = true if it is inverted
      void addSingleCNF(Var va, bool fa) {
         vec<Lit> lits;
         Lit la = fa? ~Lit(va): Lit(va);
         lits.push(la);
         _solver->addClause(lits); lits.clear();
